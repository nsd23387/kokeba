...
- Market validation: `node scripts/market/validate-topic.mjs <book-dir>` (demand-vs-competition opportunity score 0–100 + go/no-go verdict for a heritage-language title → `market.json`. Uses **live DataForSEO** data when `DATAFORSEO_LOGIN`/`DATAFORSEO_PASSWORD` are set — Google Ads search volume for demand + Labs bulk keyword-difficulty for competition, probing the book's validated keyword slots — and falls back to a deterministic heuristic otherwise. `--no-live` forces heuristic. Worker runs it in the **research** stage so weak topics surface before you invest in art.)
